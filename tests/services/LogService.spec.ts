...
describe("LogService", function () {
    ...
    describe("getList()", function () {
        test("Should correctly return paginated list result", async function () {
            ...
            respond(
                http.get("*/api/logs?page=2&perPage=1&q1=abc", () =>
                    HttpResponse.json(replyBody),
                ),
            );
    ...
    describe("getOne()", function () {
        test("Should return single log", async function () {
            respond(
                http.get("*/api/logs/test%3F123", () =>
                    HttpResponse.json({ id: "test?123" }),
                ),
            );
        ...
        test("Should return array with date grouped logs", async function () {
            respond(
                http.get("*/api/logs/stats", () =>
                    HttpResponse.json([{ total: 123, date: "2022-01-01 00:00:00" }]),
                ),
            );
