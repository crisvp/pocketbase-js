...
const delayedResponse = () => new Promise<HttpResponse>(resolve => setTimeout(() => resolve(HttpResponse.json({})), 1));
...
describe('Client', function () {
  ...
  describe('constructor()', function () {
    ...
    test('Should load all api resources', async function () {
      const baseServices: (keyof Client)[] = ['admins', 'collections', 'logs', 'settings', 'realtime'];
  ...
  describe('send()', function () {
    test('Should build and send http request', async function () {
      ...
      respond(
        ...
        http.post('*/multipartAuto', () => HttpResponse.json('successMultipartAuto'))
      );
