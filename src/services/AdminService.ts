...
export class AdminService extends CrudService<AdminModel> {
  ...
  async authWithPassword(email: string, password: string, options: AuthOptions = {}): Promise<AdminAuthResponse> {
    ...
    let authData = await this.client.send<AdminAuthResponse>(this.baseCrudPath + '/auth-with-password', request);
  ...
  async authRefresh(options?: CommonOptions): Promise<AdminAuthResponse> {
    ...
    const result = await this.client.send<AdminAuthResponse>(this.baseCrudPath + '/auth-refresh', options);
  ...
  async requestPasswordReset(email: string, options?: CommonOptions): Promise<boolean> {
    ...
    return this.client.send(this.baseCrudPath + '/request-password-reset', options).then(() => true);
  }
  ...
  async confirmPasswordReset(
    ...
    return this.client.send(this.baseCrudPath + '/confirm-password-reset', options).then(() => true);
  }
}
