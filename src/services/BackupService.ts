...
export class BackupService extends BaseService {
   ...
   * @throws {ClientResponseError}
   */
  async upload(bodyParams: Record<string, unknown> | FormData, options?: CommonOptions): Promise<boolean> {
    options = Object.assign(
      {
        method: 'POST',
        body: bodyParams,
  ...
  async delete(key: string, options?: CommonOptions): Promise<boolean> {
    ...
    return this.client.send(`/api/backups/${encodeURIComponent(key)}`, options).then(() => true);
  }
  ...
  async restore(key: string, options?: CommonOptions): Promise<boolean> {
    ...
    return this.client.send(`/api/backups/${encodeURIComponent(key)}/restore`, options).then(() => true);
  }
